...
namespace {
// Ported from UEVRJ (Framework.cpp should_suppress_openxr_rehook_guard). Under embedded RenderDoc the
// OpenXR session can sit in READY/SYNCHRONIZED/VISIBLE/FOCUSED for a while before it submits its first
// frame, and the game's desktop present pauses during that VR/stereo transition. The hook-monitor's
// rehook-if-not-presenting watchdog reads that pause as a lost hook and re-hooks D3D12 — which
// re-creates the dummy device and re-installs the present hook, clobbering the in-flight OpenXR session
// so frames never submit and VR wedges. Suppress the rehook whenever a live OpenXR session exists so
// the original (RenderDoc-coexisting) hook stays intact and OpenXR can finish coming up. Returns false
// when there is no live OpenXR session, so normal hooking is completely unaffected (no RenderDoc gate
// needed — this is self-limiting to the dangerous window).
bool should_suppress_openxr_rehook_guard() {
    ...
    const bool has_session = openxr->instance != XR_NULL_HANDLE && openxr->session != XR_NULL_HANDLE;
    if (!has_session) {
        return false;
    }

    // The session has reached READY (or beyond) and is trying to run its frame loop — fragile, do not
    // rehook underneath it. (Our fork lacks UEVRJ's finer got_first_valid_poses/last_successful_* state,
    // so we conservatively guard the whole READY..FOCUSED window plus any begin/submit activity.)
    const bool alive =
        openxr->session_ready || openxr->frame_began || openxr->ever_submitted ||
        openxr->session_state == XR_SESSION_STATE_READY ||
    ...
    if (alive) {
        SPDLOG_INFO_EVERY_N_SEC(2,
            "[Framework] Suppressing D3D rehook while OpenXR session is active/initializing (state={} ready={} frame_began={} ever_submitted={})",
            static_cast<int>(openxr->session_state), openxr->session_ready, openxr->frame_began, openxr->ever_submitted);
        return true;
    }
...
void Framework::hook_monitor() {
    ...
    // Under embedded RenderDoc, NEVER run the rehook-if-not-presenting death spiral. Re-hooking
    // re-creates the dummy device and re-installs the present hook, which clobbers RenderDoc's wrapped
    // present so the game's frames stop routing through UEVR -> VR/OpenXR init never completes
    // (xrCreateSession never fires) and the loop self-perpetuates (observed: "Last chance encountered"
    // every ~11s, stereo never engages). RenderDoc is resident before the first device (the launcher
    // guarantees it), so the original present hook already coexists with RenderDoc's wrapper; a stale
    // present here is a false alarm (the VR/stereo transition just moved the present path), not a lost
    // hook. Keep the watchdog fresh and skip rehooking. Gated on UEVR_RENDERDOC_BOOTSTRAP (launcher-set).
    static const bool s_renderdoc_bootstrap = []() {
        char v[8]{};
        ...
        || (renderer_type == Framework::RendererType::D3D12 && d3d12 != nullptr && !d3d12->is_inside_present()))
    {
        ...
        if (now - m_last_present_time >= std::chrono::seconds(5)) {
            ...
            if (!m_has_last_chance && now - m_last_chance_time > std::chrono::seconds(1)) {
                // Ported from UEVRJ: don't rehook while an OpenXR session is alive — under embedded
                // RenderDoc the desktop present pauses during the VR/stereo transition and a rehook
                // would clobber the in-flight session (frames never submit, VR wedges). Also skip under
                // embedded RenderDoc once initialized but before a session exists (the VR-init window).
                if (should_suppress_openxr_rehook_guard() || (s_renderdoc_bootstrap && m_initialized)) {
                    // NOTE: the cross-thread recover_wedged_frame() call that used to live here is removed.
                    // It was treating a SYMPTOM of the real bug (the game thread deadlocking in
                    // safetyhook::create_mid under RenderDoc — now fixed in FFakeStereoRenderingHook). With
                    // the deadlock gone the render thread runs its own frame loop; force-closing a frame
                    // from this monitor thread only corrupts the render thread's frame state (it sets
                    // frame_began=false mid-frame), which itself wedges the session in READY. So we only
                    // suppress the destructive rehook here and let the render thread own its frames.
