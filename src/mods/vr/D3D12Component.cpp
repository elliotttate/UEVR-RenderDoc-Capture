...
vr::EVRCompositorError D3D12Component::on_frame(VR* vr) {
    ...
    if (provider_velocity.texture) {
        auto desc = provider_velocity.texture->GetDesc();
        spdlog::info("Format:{}", (uintptr_t)desc.Format);
    }
    ...
    if (is_using_afw && eyeFrameBuffer.color.pTexture && vr->depthDesc[nEye].pTexture) {
        ...
        params.MotionVectorsType = (vr->is_ghosting_fix_enabled() || combined_velocity) ? Normal : FromOtherEye;
