...
exposes them through a small, versioned C ABI so AFW can ask for buffers instead of depending on
DLSS interception.

The plugin is deliberately only a provider. PureDark's existing AFW path remains the consumer; the
UEVR core bridge only queries this ABI, sizes the existing PDAFW input textures, and copies the
provided resources into those inputs when the bridge is enabled.

## What it does today

- Plugin skeleton + core state machine + exported C ABI (`include/UEVRFrameResourcesAPI.h`).
- `RenderTargetPoolProvider`: resolves `SceneDepthZ` and velocity-name candidates
  (`SceneVelocity` / `GBufferVelocity` / `Velocity`) to `ID3D12Resource*`.
- `D3D12BindProvider`: plugin-owned inline hooks on `CreateRenderTargetView` /
...
- `NgxDlssProvider`: optional detection-only DLSS observer, never required.
...
- **In-game (DLSS off):** deploy the DLL, launch with `UEVR_FRAME_RESOURCES_LOG=2
  ...
  log for `api ready`, `core selftest`, and `d3d12 depth/velocity candidate` lines.
